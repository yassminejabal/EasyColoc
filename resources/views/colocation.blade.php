...
.btn-danger{background:rgba(239,68,68,0.1);color:#f87171;border:1px solid rgba(239,68,68,0.2)}.btn-danger:hover{background:rgba(239,68,68,0.2)}
...
.badge-green{background:rgba(16,185,129,0.15);color:#34d399}.badge-orange{background:rgba(245,158,11,0.15);color:#fbbf24}.badge-blue{background:rgba(59,130,246,0.15);color:#60a5fa}
...
.meta-chip{display:flex;align-items:center;gap:0.4rem}
...
/* Invite */
.invite-box{background:var(--card2);border:1px dashed rgba(99,179,237,0.25);border-radius:10px;padding:1.25rem;display:flex;align-items:center;gap:1rem;margin-top:0.75rem}
.invite-link{font-family:'JetBrains Mono',monospace;font-size:0.8rem;color:var(--soft);flex:1;word-break:break-all}
.invite-copy{background:var(--primary-g);color:#fff;border:none;padding:0.45rem 0.9rem;border-radius:6px;font-size:0.8rem;font-weight:600;cursor:pointer;font-family:'Plus Jakarta Sans',sans-serif;white-space:nowrap;transition:opacity 0.2s}
.invite-copy:hover{opacity:0.85}
...
.cat-del{color:var(--muted);cursor:pointer;font-size:0.9rem;transition:color 0.2s}.cat-del:hover{color:var(--danger)}
</style>
</head>
<body>
<aside class="sidebar">
...
<main class="main">
  <div class="page-header">
    ...
    <button type="submit" class="btn btn-danger btn-sm">
      ...
      <a href="{{route('expense.create')}}" class="btn btn-ghost">+ Nouvelle dépense</a>
  ...
  <div class="coloc-hero">
    ...
    <div class="coloc-hero-meta">
      ...
      {{-- <span class="meta-chip">📍 le 01/09/2025</span> --}}
      {{-- <span class="meta-chip">👥 4 membres</span>
      <span class="meta-chip">💸 4 320€ dépensés</span> --}}
    </div>
  </div>
  ...
  <div class="grid-2">
    ...
    <div class="card">
      <div class="card-header">
        ...
        <button class="btn btn-primary btn-sm" onclick="document.getElementById('inviteSection').style.display='block'">+ Inviter</button>
      </div>
      ...
      <div class="member-row">
        <div class="member-av" style="background:linear-gradient(135deg,#10b981,#059669)">MA</div>
        <div class="member-info">
          <div class="member-name">Marie Dupont</div>
          <div class="member-rep"><span class="rep-star">★★★★</span> Réputation : 4 · Rejoint le 05/09/2025</div>
        </div>
        <form method="POST" action="/colocations/1/members/2/remove">
          @csrf @method('DELETE')
          <button type="submit" class="btn btn-danger btn-sm">Retirer</button>
        </form>
      </div>
      <div class="member-row">
        <div class="member-av" style="background:linear-gradient(135deg,#f59e0b,#d97706)">TO</div>
        <div class="member-info">
          <div class="member-name">Tom Martin</div>
          <div class="member-rep"><span class="rep-star">★★★</span> Réputation : 3 · Rejoint le 10/09/2025</div>
        </div>
        <form method="POST" action="/colocations/1/members/3/remove">
          @csrf @method('DELETE')
          <button type="submit" class="btn btn-danger btn-sm">Retirer</button>
        </form>
      </div>
      <div class="member-row">
        <div class="member-av" style="background:linear-gradient(135deg,#ec4899,#be185d)">JU</div>
        <div class="member-info">
          <div class="member-name">Julie Bernard</div>
          <div class="member-rep"><span class="rep-star">★★★★★</span> Réputation : 5 · Rejoint le 15/09/2025</div>
        </div>
        <form method="POST" action="/colocations/1/members/4/remove">
          @csrf @method('DELETE')
          <button type="submit" class="btn btn-danger btn-sm">Retirer</button>
        </form>
      </div>
      ...
      <div id="inviteSection" style="display:none;margin-top:1rem">
        <div style="font-size:0.85rem;font-weight:600;color:var(--soft);margin-bottom:0.5rem">🔗 Lien d'invitation</div>
        <div class="invite-box">
          <div class="invite-link">https://easycoloc.fr/join/abc123xyz789token</div>
          {{-- <button class="invite-copy" onclick="navigator.clipboard.writeText('https://easycoloc.fr/join/abc123xyz789token');this.textContent='Copié ✓'">Copier</button> --}}
        </div>
        <div style="margin-top:0.75rem;font-size:0.82rem;color:var(--muted)">Ou inviter par email :</div>
        <form method="POST" action="/colocations/1/invite" style="display:flex;gap:0.5rem;margin-top:0.4rem">
          @csrf
          <input type="email" name="email" placeholder="ami@exemple.fr" style="flex:1;background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:0.6rem 0.9rem;color:var(--text);font-family:'Plus Jakarta Sans',sans-serif;font-size:0.88rem;outline:none">
    ...
    <div>
      <div class="card">
        ...
        <form action="{{ route('categorie.destroy', $item->id) }}" method="POST" style="display:inline-flex;align-items:center;margin-right:0.2rem;">
            @csrf
            @method('POST') 
            
            <span class="cat-tag" style="display:flex;align-items:center;gap:0.2rem;cursor:pointer;">
                🛒 {{ $item->name }}
                <button type="submit" class="cat-del" style="background:none;border:none;color:red;cursor:pointer;">✕</button>
            </span>
        </form>
    @endforeach
</div>
        <form method="POST" action="{{route('categorie.store')}}" style="display:flex;gap:0.5rem">
          @csrf
          <input type="text" name="name" placeholder="Nouvelle catégorie..." style="flex:1;background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:0.6rem 0.9rem;color:var(--text);font-family:'Plus Jakarta Sans',sans-serif;font-size:0.88rem;outline:none">
      ...
      <div class="card" style="border-color:rgba(239,68,68,0.2);background:rgba(239,68,68,0.04)">
        <div class="card-header" style="margin-bottom:0.75rem">
          <span class="card-title" style="color:#f87171">⚠️ Zone de danger</span>
        </div>
        <p style="font-size:0.85rem;color:var(--muted);margin-bottom:1rem">Quitter ou annuler la colocation affectera votre réputation si vous avez des dettes actives.</p>
        <div style="display:flex;gap:0.75rem;flex-wrap:wrap">
          <form method="POST" action="/colocations/1/leave">
            @csrf
            <button type="submit" class="btn btn-ghost btn-sm">Quitter la colocation</button>
          </form>
          <form method="POST" action="/colocations/1/cancel">
            @csrf 
            @method('PATCH')
